...
import { ACTION_TYPE, ACTIVATION_TYPE_ICON, PREPARED_ICON, PROFICIENCY_LEVEL_ICON, SUBCATEGORY } from "./constants.js";
...
Hooks.once("tokenActionHudCoreApiReady", async (coreModule) => {
	ActionHandler = class ActionHandler extends coreModule.api.ActionHandler {
		...
		async buildSystemActions(subcategoryIds) {
			...
			this.activationSubcategoryIds = ["actions", "bonus-actions", "crew-actions", "lair-actions", "legendary-actions", "reactions", "other-actions"];
			...
			if (this.actorType === "character" || this.actorType === "npc") {
				this.inventorySubcategoryIds = ["equipped", "consumables", "containers", "equipment", "loot", "tools", "weapons", "unequipped"];

				this._buildCharacterActions();
			}
			if (this.actorType === "vehicle") {
				this.inventorySubcategoryIds = ["consumables", "equipment", "tools", "weapons"];
		...
		_buildConditions() {
			...
			const actions = conditions.map((condition) => {
				...
				const active = this.actors.every((actor) => {
					...
					return effects.map((effect) => effect.flags?.core?.statusId).some((statusId) => statusId === id);
				})
					? " active"
					: "";
		...
		_buildRests() {
			...
			// Get actions
			const actions = Object.entries(restTypes).map((restType) => {
				const id = restType[0];
		...
		_buildSpells() {
			...
			for (const subcategoryId of this.spellSubcategoryIds) {
				...
				// Build activations
				if (this.activationSubcategoryIds) {
		...
		_buildUtility() {
			...
			const actions = Object.entries(utilityTypes).map((utilityType) => {
				...
				if (utilityType[0] === "inspiration") {
					const active = this.actors.every((actor) => actor.system.attributes?.inspiration) ? " active" : "";
					cssClass = `toggle${active}`;
				}
				return {
					id,
		...
		_getAction(actionType, entity) {
			...
			if (entity?.system?.recharge && !entity?.system?.recharge?.charged && entity?.system?.recharge?.value) {
		...
		_isUsableSpell(spell) {
			...
			const preparationModes = Object.keys(game.dnd5e.config.spellPreparationModes).filter((preparationMode) => preparationMode !== "prepared");
