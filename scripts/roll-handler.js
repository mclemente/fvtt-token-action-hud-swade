...
Hooks.once("tokenActionHudCoreApiReady", async (coreModule) => {
	RollHandler = class RollHandler extends coreModule.api.RollHandler {
		 ...
		 */
		_needsRecharge(item) {
			return item.system.recharge && !item.system.recharge.charged && item.system.recharge.value;
		}
		...
		async _toggleCondition(event, token, actionId, effect = null) {
			...
			if (game.dfreds && effect?.flags?.isConvenient) {
				...
				isRightClick ? await token.toggleEffect(condition, { overlay: true }) : await token.toggleEffect(condition);
			}
